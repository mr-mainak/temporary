...
to access PIG
GO to the same username hdoop and start the hadoop using start-all.sh command
after that run this command to start jon history tracker needed by PIG to track JOB
mr-jobhistory-daemon.sh start historyserverthen it will start the job history daemon & after that check that using jps command
now start the pig using
pig
else
pig -x local
...
Exam guide
To access hadoop
change the username in the system
use this command
su - hdoop
the password is
root
now run this code without going to any folder
jps
To check if there is any running hadoop else
start-all.sh
and all the hdoop nodes should start runnig

to access mongo
run this command to get the container ID
sudo docker ps
this will give the running container
Copy the container ID where mongo_stack_mongo_1 is written as name
after that copy the container ID and run this code
sudo docker exec -it <conatiner_ID> /bin/bash
this will make you enter to the mongo docker
there run this command to enter into mongo sh
mongosh -u root -p root
you will be getting the mongo bash
...
path = "/home/mainak/HDD_PC/Datasets/guns.csv"
df = spark.read.option("header",'True').option('delimiter', ',').csv(path)
df.printSchema()

df.filter("number_courses=='8'").show()
